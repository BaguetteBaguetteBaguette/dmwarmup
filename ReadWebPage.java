...
public class ReadWebPage {
   public static void main(String[] args) throws IOException, InterruptedException {
      ...
      while (counter <= 96) {
         ...
         // output to a file so its easy to mess with (you won’t be for your finished
         // program)
         FileOutputStream fs = new FileOutputStream("mainFile.txt");
         PrintWriter pw = new PrintWriter(fs);
         pw.println(response.body()); // response.body() is the html source code in a string format. It outputs to a
                                      // file so you can see it easier right now, but you will ultimately want to just
                                      // manipulate the strings a lot
         pw.close();
         ...
         // Resourcing Arraylist
          // [author0, path1, name2, servings3, ingredients4, instructions5]
         ...
         while (mainReader.hasNextLine()) {
            String[] arrayHolder = new String[6];
            ...
            if (line.contains("thumb-link\" href=\"https://")) {
               ...
               for (int i = 0; i < 1000; i++) // skip all the lines that don't really matter
               {
               ...
               while (recipeReader.hasNextLine()) {
                  ...
                  // Get the author of the recipe, and add it to the array for the csv file
                  ...
                  // Get the breadcrumb trail (path) for the recipe, and make them into one string
                  // that can be added to the array for the csv file
                  ...
                  // Get the number of servings for the recipe, and add it to the array for the
                  // csv file
                  ...
                  // Get the ingredients for the recipe, and make them into one string that can be
                  // added to the array for the csv file
                  stop = false;
                  String shortRec;
                  ...
                  // Get the procedure (instructions) for the recipe, and make them into one
                  // string that can be added to the array for the csv file
                  stop = false;
                  ...
                  if (lineRecipe.contains("recipe-details-procedure")) {
                     while (recipeReader.hasNextLine() && !stop) {
                        ...
                        if (lineRecipe.contains("</div>")) {
                        ...
                        } else if (lineRecipe.contains("<b>") || lineRecipe.contains("<i>")
                              || lineRecipe.contains("<br>") || lineRecipe.contains("<li>")
                              || lineRecipe.contains("<ol>")) {
                           ...
                           lineRecipe = lineRecipe.replace("</ol>", "");
                           lineRecipe = lineRecipe.replace("</em>", "");
                           lineRecipe = lineRecipe.replace("<em>", "");
                        }
               ...
               rows.add(arrayHolder); // send the array to CSVPrinter
